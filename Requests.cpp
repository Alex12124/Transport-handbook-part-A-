...
void ModifyStopRequest::ParseFrom(std::string_view input) {
  ...
  longitude = ConvertToDouble(input) * 3.1415926535 / 180;
}
...
void PrintResponses(const std::string bus_name, optional<BusStats> stats,
		ostream& stream) {
  if(!stats) {
  ...
  stream << "Bus " << bus_name  << ": "<< stats->stop_count
		  ...
		  << " unique stops, " << stats->route_distance << " route length\n";
  }
}
//-----------------------PrintResults-------------------------------//
...
void Visitor::Visit(const ReadBusRequest& request) const {
  PrintResponses(request.bus_name, rm->GetBusStats(request.bus_name), cout);
}
void Visitor::Visit(const ModifyBusRequest& request) const {
  ...
  rm->SetBusRoute(request.bus_name, request.stops);
}
void Visitor::Visit(const ModifyStopRequest& request) const {
  rm->SetStopCoords(request.stop_name, Coords{request.latitude, request.longitude});
}
